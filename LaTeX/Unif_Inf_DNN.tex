...
Section \ref{sec:JK_BS_IOUS} presents general results on the consistency of Jackknife and Bootstrap variance estimators for infinite-order U-statistics developed to obtain the results of the previous section.
Section \ref{sec:unif_inf} extends results from \citet{ritzwoller_uniform_2024} to the TDNN estimator and thus introduces methods to construct uniform confidence bands for the TDNN estimator.
Section \ref{sec:simulations} explores the performance of the developed uniform inference methods in a number of setups and Section \ref{sec:application} applies them to the context of {\color{red} LOREM IPSUM}.
...
\newpage
\section{Jackknife and Bootstrap Variance Estimators for IOUS}\label{sec:JK_BS_IOUS}
\hrule

\begin{thm}[Consistency of Jackknife Variance Estimators for IOUS]\label{thm:JK_IOUS_Cons}\mbox{}\\*
	
\end{thm}

\begin{thm}[Consistency of Bootstrap Variance Estimators for IOUS]\label{thm:BS_IOUS_Cons}\mbox{}\\*
...
{\color{red} Ideas:
	\begin{enumerate}
		\item Nonparametric IV (use TDNN for first stage)
		\item ``Mostly Harmless Machine Learning: Learning Optimal Instruments in Linear IV Models'' (2021) by Jiafeng Chen, Daniel L. Chen, Greg Lewis
		\item Use for Double-Debiased Machine Learning
	\end{enumerate}
}
...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			...
			+ 2 \sum_{j = 2}^{s_2} \frac{j}{s_2}\frac{n}{s_2}\left(\frac{s_2 e}{j}\frac{j}{n}\right)^{j}
			\left[\binom{s_2}{j}V_{s_1, s_2}^{j}\right]
			%
			= \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ 2e \sum_{j = 2}^{s_2} \frac{j}{s_2} \left(\frac{e s_2}{n}\right)^{j-1}
			\left[\binom{s_2}{j}V_{s_1, s_2}^{j}\right]                                                                                                \\
			%
			 & \leq \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2 e}{n}\sum_{j = 2}^{s_2}\left[\binom{s_2}{j}V_{s_1, s_2}^{j}\right]
			+ \frac{2 e}{n}\sum_{j = 2}^{s_2} (j-1)\left(\frac{s_2 e}{n}\right)^{j-1}
			\left[\binom{s_2}{j}V_{s_1, s_2}^{j}\right]                                                                                                \\
			%
			 & \leq \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ \frac{2e}{n}\sum_{j = 2}^{s_2} (j-1)\left(\frac{s_2e}{n}\right)^{j-1}\zeta_{s_1, s_2}^{s_2}                                              \\
			%
			 & \leq \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ \frac{2e}{n}\zeta_{s_1, s_2}^{s_2}\sum_{j = 1}^{\infty} j \left(\frac{s_2 e}{n}\right)^{j}                                               \\
			%
			 & = \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ \frac{2 s_2}{n^2}\zeta_{s_1, s_2}^{s_2}\frac{e^2}{\left(\frac{s_2 e}{n} - 1\right)^2}                                                    \\
			%
			 & \lesssim \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ \frac{2 s_2}{n^2}\frac{e^2}{\left(\frac{s_2 e}{n} - 1\right)^2} \, \cdot \, \left(\mu^2(\mathbf{x}) + \sigma_{\varepsilon} + o(1)\right) \\
			%
			 & \lesssim \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ o(1)
	...
	\begin{equation*}
		...
		\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right) \sim s_2^{-1}
	\end{equation*}
	Using these findings, consider the following expression.
	\begin{equation}
		\begin{aligned}
			\frac{\E\left[T_i^2\right]}{V_{s_1, s_2}^{1}\left(\mathbf{x}\right)}
			= \frac{\E\left[T_i^2\right]}{\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)}
			 & \lesssim \frac{\frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
				+ \frac{2e}{n}\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ o(1)}{\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)}                    \\
			%
			 & \leq \frac{\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)
				+ 2e\left(\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}\right)
			+ o(1)}{\left(n-1\right)\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)}    \\
			%
			 & = \frac{1}{n-1}
			+ \frac{2e}{n-1}\frac{\zeta_{s_1, s_2}^{s_2} - s_2 \zeta_{s_1, s_2}^{1}}{\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)}
			+ o(1)
			\rightarrow 0.
		\end{aligned}
	\end{equation}
	...
	\begin{equation}
		...
		\end{aligned}
	\end{equation}
	{\color{red} Continue as in \citet{peng_bias_2021} - shows ratio consistency of JK Var estimator}
\end{proof}
...
\hrule
\subsection{Useful Lemmas for Proofs for Results in Section \ref{sec:unif_inf}}
