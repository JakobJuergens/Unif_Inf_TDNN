...
Consider as an example their bootstrap variance estimator.
...
\begin{corollary}
	...
	\begin{equation}
		...
		\left(\hat{\mu}_{s_1, s_2}(\mathbf{x}; \mathbf{D}_n) - \mu(\mathbf{x}) - \frac{b}{n} \sum_{i=1}^n h^{(1)}_{s_1, s_2}(\mathbf{x}; \mathbf{z}_{i})\right)
		%
		= \sqrt{\frac{n}{{s_2}^2 \sigma_{s_2}^2}} \operatorname{HR}_{s_1, s_2}(\mathbf{x}; \mathbf{D}_n)
		%
		\lesssim \xi_{n, s},
		\quad \text {where}
	\end{equation}
