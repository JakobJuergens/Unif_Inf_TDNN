...
\begin{lem}[DNN Haj\'ek Kernel Expectation]\label{lem:psi_s_1}\mbox{}\\*
	...
	\begin{equation}
		...
		= \left(\mu\left(\mathbf{x}_1\right) + \varepsilon_1\right) \left(1 - \psi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x} \|\right)\right)\right)^{s-1}
		+ (s-1)\E_{D}\left[\mu\left(\mathbf{X}_2\right)\kappa\left(\mathbf{x}; \mathbf{Z}_2, D\right)  \, \Big| \, \mathbf{Z}_1 = \mathbf{z}_1 \right].
	\end{equation}
\end{lem}
...
\begin{proof}[Proof of Lemma \ref{lem:psi_s_1}]
	\begin{equation}
		\begin{aligned}
			...
			%
			 & = y_1 \left(1 - \psi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x} \|\right)\right)\right)^{s-1}
			 + (s-1)\E_{D}\left[\kappa\left(\mathbf{x}; \mathbf{Z}_2, D\right) Y_2 \, \Big| \, \mathbf{Z}_1 = \mathbf{z}_1 \right]\\
			 %
			 & = \left(\mu\left(\mathbf{x}_1\right) + \varepsilon_1\right) \left(1 - \psi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x} \|\right)\right)\right)^{s-1}
			 + (s-1)\E_{D}\left[\mu\left(\mathbf{X}_2\right)\kappa\left(\mathbf{x}; \mathbf{Z}_2, D\right)  \, \Big| \, \mathbf{Z}_1 = \mathbf{z}_1 \right]
		\end{aligned}
...
\begin{proof}[Proof of Lemma \ref{lem:Cov_TDNN_k}]
	...
	Recall Lemma \ref{lem:psi_s1s2_1} and observe the following.
	\begin{equation}
		\begin{aligned}
			\psi_{s_1, s_2}^{1}\left(\mathbf{x}; \mathbf{z}_1\right)
		\end{aligned}
