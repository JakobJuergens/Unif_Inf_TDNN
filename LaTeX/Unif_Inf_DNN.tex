...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			...
			%
			& = \frac{1}{n-1} V_{s_1, s_2}^{1}\left(\mathbf{x}\right)
			+ \frac{n^2}{s_2^2}\sum_{j = 2}^{s_2} \frac{j}{n} \binom{s_2}{j}
			\left(1 + \frac{4 j^2}{\left(n - 2j\right)^2}\right)\binom{n}{j}^{-1} \left[\binom{s_2}{j}V_{s_1, s_2}^{j}\right]
		\end{aligned}
	\end{equation}
	Then for $s_2 = o(n)$ and $n$ sufficiently large, we have the following.
	{\color{red} Continue HERE! There is a relatively simple (meaning similar to \citet{peng_bias_2021}) asymptotic bound that allows us to use the same trick.}
\end{proof}
