...
\begin{proof}
	...
	\begin{equation}
		\begin{aligned}
			...
			\right)^2\\
			%
			& = \frac{n-1}{n} \sum_{i = 1}^{n}\left(\sum_{j = 1}^{s_2}\binom{s_2}{j}\binom{n}{j}^{-1}
			\left( \sum_{\ell \in L_{j}\left([n]\backslash \{i\}\right)} h^{(j)}_{s_1, s_2}(\mathbf{x}; D_{\ell})
				- \sum_{\iota \in L_{n,j}} h^{(j)}_{s_1, s_2}(\mathbf{x}; D_{\iota})\right)
			\right)^2\\
			%
			& = {\color{red} LOREM IPSUM}
		\end{aligned}
