...
\begin{thm}[\citet{demirkaya_optimal_2024} - Theorem 1]\label{thm_dem1}\mbox{}\\*
...
\end{equation}
As a direct consequence of Theorem \ref{thm_dem1}, this leads to the elimination of the first-order bias term leading to desirable finite-sample properties.
Furthermore, the authors show that this construction improves the quality of the normal approximation.


\begin{thm}[\citet{demirkaya_optimal_2024} - Theorem 3]\label{thm_dem3}\mbox{}\\*
	Assume that Conditions (1) to (3) from Theorem \ref{thm_dem1} hold.
...
\begin{lem}[Kernel of TDNN Estimator - Adapted from Lemma 8 of \citet{demirkaya_optimal_2024}]\label{lem_dem8}\mbox{}\\*
...
\begin{lem}[\citet{demirkaya_optimal_2024} - Lemma 7]\label{lem_dem7}\mbox{}\\*
	For any fixed $\mathbf{x}$ we have the following as $s \rightarrow \infty$ and $s = o(n)$
	\begin{equation}
		\lim_{n \rightarrow \infty} n^{-1}\frac{\xi_{s}^{s}\left(\mathbf{x}\right)}{\xi_{s}^{1}\left(\mathbf{x}\right)}
		= 0.
	\end{equation}
\end{lem}
...
While Theorem \ref{thm_dem3} allows $s_2$ to be of the order $o(n)$, the variance estimators rely on the considerably stronger condition that $s_2 = o(n^{1/3})$.
...
\begin{lem}[\citet{peng_bias_2021} - Theorem 6]\label{thm_peng21_6}\mbox{}\\*
...
Thus, we need to show an analog of Lemma \ref{lem_dem7} for the TDNN kernel.
...
\begin{thm}[\citet{ritzwoller_uniform_2024} - Theorem 4.1]\label{thm_rit4_1}\mbox{}\\*
...
\begin{thm}[Adapted from \citet{ritzwoller_uniform_2024} - Theorem 4.2]\label{thm_rit4_2}\mbox{}\\*
	Define the terms
	\begin{equation}
		\bar{\psi}_{s_2}^2
...
\subsection{Useful Lemmas for Proofs for Results in Section \ref{pw_inf}}
\hrule
...
\begin{proof}[Proof of Lemma \ref{Var_TDNN_k}]
...
\begin{lem}[Kernel Covariances of the TDNN Kernel]\label{Cov_TDNN_k}
	For the kernel of the TDNN estimator with subsampling scales $s_1$ and $s_2$, it holds that
	\begin{equation}
...
\begin{proof}[Proof of Lemma \ref{Cov_TDNN_k}]
	...
	\begin{equation}
		\begin{aligned}
			...
			\right]                                                                                                                                                                     \\
			%
			 & \quad = \binom{s_2}{s_1}^{-1}\left(\E_{2:s_2}\left[
			...
			\right)                                                                                                                                                                     \\
			%
			 & \quad = \binom{s_2}{s_1}^{-1}\binom{s_2 - 1}{s_1} s_1 \E_{2:(s_1 + 1)}\left[\kappa(\mathbf{x}; \mathbf{Z}_2, D_{2:(s_1 + 1)}) Y_2\right]                                 \\
			 & \quad \quad + \binom{s_2}{s_1}^{-1}\binom{s_2 - 1}{s_1 - 1}\left(
			...
			\E_{2}\left[\mu(\mathbf{X}_2) \, (s_1 - 1) \, \E_{3:s_1}\left[\kappa(\mathbf{x}; \mathbf{Z}_2, D_{[s_1]}) \, | \, \mathbf{Z}_2\right] \, \Big| \, D_1 = \mathbf{Z}_1\right] \\
			 & \quad \quad + \binom{s_2}{s_1}^{-1}\binom{s_2 - 1}{s_1 - 1}\mu(\mathbf{X}_1)
	...
	\begin{equation}
		\begin{aligned}
			...
			%
			     & \quad = \E_{2:s_2}\left[\sum_{i = 1}^{s_2} \kappa(\mathbf{x}; \mathbf{Z}_i, D) Y_i \, \Big| \, D_1 = \mathbf{Z}_1\right]           \\
			%
			     & \quad = \E_{2:s_2}\left[\kappa(\mathbf{x}; \mathbf{Z}_1, D) Y_1 \, \Big| \, D_1 = \mathbf{Z}_1\right]
			+ \E_{2:s_2}\left[\sum_{i = 2}^{s_2} \kappa(\mathbf{x}; \mathbf{Z}_i, D) Y_i \, \Big| \, D_1 = \mathbf{Z}_1\right]                        \\
			%
			     & \quad = \mu(\mathbf{X}_1) \left(1 - \varphi\left(B\left(\mathbf{X}, \| \mathbf{X}_1 - \mathbf{x} \|\right)\right)\right)^{s_2 - 1}
			...
			%
			     & \quad = \mu(\mathbf{X}_1) \left(1 - \varphi\left(B\left(\mathbf{X}, \| \mathbf{X}_1 - \mathbf{x} \|\right)\right)\right)^{s_2 - 1} \\
			     & \quad \quad + \P\left(\|\mathbf{X}_2 - \mathbf{x}\| < \|\mathbf{X}_1 - \mathbf{x}\|\right)\E_{2}\left[
				\mu(\mathbf{X}_2)\, (s_2 - 1) \,
				\left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_2 - \mathbf{x} \|\right)\right)\right)^{s_2 - 2}
				\, \Big| \, \|\mathbf{X}_2 - \mathbf{x}\| < \|\mathbf{X}_1 - \mathbf{x}\|\right]
		\end{aligned}
	\end{equation}
	Plugging into the original expression, we obtain the following.
	\begin{equation}
		\zeta_{s_1, s_2}^{1}\left(\mathbf{x}\right)
