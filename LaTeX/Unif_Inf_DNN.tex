...
\begin{proof}[Proof of Lemma \ref{lem:Cov_TDNN_k}]
	...
	\begin{equation}
		\begin{aligned}
			...
			% %
			%  & \geq \sigma_{\varepsilon}^{2}\cdot
			%  \Var\left(w_{1}^{*} \left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x}\|\right)\right)\right)^{s_1 - 1}
			%  + w_{2}^{*}\left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x}\|\right)\right)\right)^{s_2 - 1}\right)   \\
			%  % 
			%  & \geq \sigma_{\varepsilon}^{2} \cdot \Var\left(\left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x}\|\right)\right)\right)^{s_2 - 1}\right)   \\                                             
			%  %
			%  & = \sigma_{\varepsilon}^{2} \cdot \left[
			% 	\E\left[\left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x}\|\right)\right)\right)^{2(s_2 - 1)}\right]
			% 	- \left(\E\left[\left(1 - \varphi\left(B\left(\mathbf{x}, \| \mathbf{X}_1 - \mathbf{x}\|\right)\right)\right)^{s_2 - 1}\right]\right)^2
			%  \right]
		\end{aligned}
	\end{equation}
