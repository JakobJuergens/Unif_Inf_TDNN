...
\vspace{-0.3cm}
\begin{center}
	{\small Supplementary material is available at: {\color{red} LOREM IPSUM}}
\end{center}
\vspace{0.3cm}
\hrule
\singlespacing
...
To achieve this goal, we use novel results on uniform inference for infinite-order U-statistics developed in \citet{ritzwoller_uniform_2024}.
...
\end{align}
Given a subsampling scale $s$ satisfying $1 \leq s \leq n$, a generic set of subsample indices $\ell \in L_{n,s}$ and a corresponding generic subset of our data $D_{\ell} = \{\mathbf{Z}_i \, | \, i \in \ell\}$, we can consider an analogous ordering of $D_s$.
...
Noteworthy properties of $\kappa$ are its permutational symmetry in $D_s$ and that $\kappa$ does not consider the response variable when assigning weights to the observations under consideration.
The latter immediately implies a property that has been called \textit{Honesty} by \citet{wager_estimation_2018}.

\vspace{0.5cm}
\begin{definition}[Symmetry and Honesty - Adapted from \citet{ritzwoller_uniform_2024}]
	\begin{enumerate}
		\item The kernel $\kappa\left(\cdot, \cdot, D_{\mathrm{s}}\right)$ is Honest in the sense that
		      $$\kappa\left(x, X_i, D_{\mathbf{s}}\right) \indep m\left(Z_i ; \mu\right) \mid X_i, D_{\mathbf{s}_{-i}},$$
		      where $\indep$ denotes conditional independence and $\mathrm{s}_{-i}$ denotes the set $\mathrm{s} \backslash\{i\}$.
		\item The kernel $\kappa\left(\cdot, \cdot, D_{\mathrm{s}}\right)$ is positive and satisfies the restriction
		      $\sum_{i \in s} \kappa\left(\cdot, X_i, D_{\mathrm{s}}\right)=1$ almost surely.
		      Moreover, the kernel $\kappa\left(\cdot, X_i, D_{\mathrm{s}}\right)$ is invariant to permutations of the data $D_{\mathrm{s}}.$
	\end{enumerate}
\end{definition}
...
\begin{definition}[Shrinkage and Incrementality - Adapted from \citet{ritzwoller_uniform_2024}]
	We say that the kernel $\kappa\left(\cdot, \cdot, D_{\mathrm{s}}\right)$ has a uniform shrinkage rate $\epsilon_b$ if
	\begin{equation}
		\sup_{P \in \mathbf{P}} \sup_{j \in[p]}
		\mathbb{E}\left[\max \left\{\left\|X_i-\mathbf{x}^{(p)}_{j}\right\|_{2}: \kappa\left(\mathbf{x}^{(p)}_{j}, X_i, D_{\mathrm{s}}\right)>0\right\}\right]
		\leq \epsilon_b .
	\end{equation}
	We say that a kernel $\kappa\left(\cdot, \cdot, D_{\mathrm{s}}\right)$ is uniformly incremental if
	\begin{equation}
		\inf_{P \in \mathbf{P}} \sup_{j \in[p]}
		\operatorname{Var}\left(\mathbb{E}\left[\sum_{i \in \mathbf{s}} \kappa\left(\mathbf{x}^{(p)}_{j}, X_i, D_{\mathbf{s}}\right) m\left(D_i ; \mu\right) \mid l \in \mathbf{s}, D_l=D\right]\right)
		\gtrsim b^{-1}
	\end{equation}
...
\begin{assumption}[Boundedness - Adapted from \citet{ritzwoller_uniform_2024}]
	...
	\end{equation}
\end{assumption}
As the authors point out, this condition can easily be replaced by a condition on the subexponential norm.
This, being more in line with the assumptions of \cite{demirkaya_optimal_2024}, is a desirable substitution.
...
The variogram
\begin{equation}
	V(x ; g)=\mathbb{E}\left[\left(m\left(Z_i ; \theta_0(x), g\right)-m\left(Z_i ; \theta_0(x), g_0\right)\right)^2 \mid X=x\right]
\end{equation}
is uniformly Lipschitz in both of its components, in the sense that
\begin{equation}
	\sup _{P \in \mathbf{P}} \sup _{g \in \mathcal{G}}\left|V(x ; g)-V\left(x^{\prime} ; g\right)\right| \lesssim\left\|x-x^{\prime}\right\|_{\infty}
\end{equation}
holds for all $x$ and $x^{\prime}$ in $\mathcal{X}$

Define the moments
\begin{equation}
	M^{(1)}(x ; \theta, g)=\mathbb{E}\left[m^{(1)}\left(D_i ; \theta, g\right) \mid X_i=x\right] \quad \text { and } \quad M^{(2)}(x ; g)=\mathbb{E}\left[m^{(2)}\left(D_i ; g\right) \mid X_i=x\right] \text {, }
\end{equation}
associated with the functions $m^{(1)}(\cdot ; \theta, g)$ and $m^{(2)}(\cdot ; g)$ introduced in Assumption 3.2. Both moments are uniformly Lipschitz in their first component, in the sense that
\begin{equation}
	\sup _{P \in \mathbf{P}} \sup _{g \in \mathcal{G}}\left|M^{(j)}(x ; g)-M^{(j)}\left(x^{\prime} ; g\right)\right| \lesssim\left\|x-x^{\prime}\right\|_{\infty}
\end{equation}
for each $j$ in $\{1,2\}$ and all $x$ and $x^{\prime}$ in $\mathcal{X}$. 
\end{assumption}
