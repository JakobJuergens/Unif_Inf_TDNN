...
\begin{equation}
	...
	= \sum_{j = 1}^{s_2}\binom{s_2}{j}H_{n,s_1, s_2}^{j}
\end{equation}
...
However, as pointed out, their consistency results rely on a likely suboptimal rate condition for the subsampling scale.
...
\begin{theorem}[\citet{ritzwoller_uniform_2024} - Theorem 4.1]
	...
	we have that
	\begin{equation}
		\sqrt{\frac{n}{\sigma_{b, j}^2 b^2}} \frac{1}{N_b} \sum_{\mathbf{s} \in \mathbf{S}_{n, b}} u\left(x^{(j)} ; D_{\mathbf{s}}\right) \rightsquigarrow \mathcal{N}(0,1),
		\quad \text{as} \quad
		n \rightarrow \infty.
	\end{equation}
\end{theorem}
...
\vspace{0.5cm}
\begin{theorem}[\citet{ritzwoller_uniform_2024} - Theorem 4.2]
