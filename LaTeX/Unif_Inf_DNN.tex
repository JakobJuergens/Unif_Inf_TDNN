...
\begin{lem}[{\color{red} From Demirkaya - Proof of Lemma 7}]\mbox{}\\*
	Let $D = \{\mathbf{Z}_1, \dotsc, \mathbf{Z}_{s}\}$ be a vector of i.i.d. random variables drawn from $P$.
	Furthermore, let
	\begin{equation}
		\Omega_{s}
		= \E\left[h_{s}\left(\mathbf{x}; \mathbf{Z}_1, \ldots,  \mathbf{Z}_{s}\right) \cdot
		h_{s}\left(\mathbf{x}; \mathbf{Z}_1, \ldots,\mathbf{Z}_{s}\right)\right].
	\end{equation}
	Then,
	\begin{equation}
		\Omega_{s}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Starting from this decomposition, we can make the following observations.
{\color{red} Demirkaya - Proof of Lemma 7}
\begin{equation}
	\begin{aligned}
		\E_{D}\left[h_{s_2}^{2}\left(\mathbf{x}; D\right)\right]
		= \E_1\left[\mu^2\left(\mathbf{X}_1\right) s \E_{2:s}\left[\kappa\left(\mathbf{x}; \mathbf{Z}_1, D\right)\right]\right] + \sigma_{\varepsilon}^2
		\lesssim \mu(\mathbf{x})^2 + \sigma_{\varepsilon}^2 + o(1) 
